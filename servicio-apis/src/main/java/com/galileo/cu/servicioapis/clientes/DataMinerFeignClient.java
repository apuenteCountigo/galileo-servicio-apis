...
public interface DataMinerFeignClient {
    ...
    @PostMapping("/API/v1/json.asmx/CreateElement")
    ConnectAppResultDataMiner salvarElementoDataMinerBaliza(URI uri, @RequestBody DataMinerBaliza dataMinerBaliza);
    ...
    ResponseEntity<?> obtenerParametrosBaliza(URI uri,
            @RequestBody GetParametersByPageForElement getParametersByPageForElement);

    /**
     * Obtener los valores de la tabla a partir de un parámetro.
     *
     * @param uri       Uri de conexión a Dataminer
     * @param parameter Valores de los parámetros para obtener la información de la
     *                  tala.
